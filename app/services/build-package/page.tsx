  ...
  return (
    <main className="min-h-screen bg-white flex flex-col pt-24 font-sans antialiased text-[#0A1128]">
      ...
      <section className="bg-gray-50 py-32 border-b border-gray-100 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
          ...
          <h1 className="text-5xl lg:text-7xl font-extrabold mb-8 tracking-tighter leading-tight">
            Architect Your <br /><span className="text-[#0A1128]">Perfect Journey.</span>
          </h1>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 -mt-20 relative z-20">
         <div className="bg-white rounded-[3rem] shadow-2xl shadow-[#0A1128]/10 overflow-hidden border border-gray-100">
            <div className="grid grid-cols-1 lg:grid-cols-3 min-h-[600px]">
               ...
               <div className="col-span-2 p-12 lg:px-20 lg:py-16 bg-white overflow-hidden">
                  {step === 1 && (
                     <div className="animate-in fade-in slide-in-from-right-10 duration-500">
                        <h2 className="text-3xl font-bold mb-4">Start with the Destination</h2>
                        <p className="text-gray-500 mb-10">Where should your journey begin?</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
                           {['Manali', 'Kashmir', 'Amalfi Coast', 'New Delhi (NCR)', 'Global'].map((city, idx) => (
                              <button key={idx} onClick={() => setStep(2)} className="w-full bg-gray-50 p-6 rounded-2xl hover:bg-[#0A1128] hover:text-white transition-all text-left font-bold flex justify-between items-center group">
                                 {city}
                  ...
                  {step === 2 && (
                     <div className="animate-in fade-in slide-in-from-right-10 duration-500">
                        <h2 className="text-3xl font-bold mb-4">Choose Your Fleet</h2>
                        <p className="text-gray-500 mb-10">Select the mode of transport that suits your style.</p>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                           {['Executive Sedan', 'SUV Fleet', 'Private Jet Charter', 'Luxury Yacht'].map((fleet, idx) => (
                              <button key={idx} onClick={() => setStep(3)} className="w-full bg-gray-50 p-6 rounded-2xl hover:bg-[#0A1128] hover:text-white transition-all text-left font-bold flex justify-between items-center group">
                                 {fleet}
                                 <svg className="w-4 h-4 text-transparent group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M9 5l7 7-7 7" strokeWidth={3} /></svg>
                              </button>
                           ))}
                        </div>
                        <button onClick={() => setStep(1)} className="mt-10 text-[#0A1128] font-bold text-sm underline underline-offset-4 decoration-[#FFB800]">← Go back</button>
                     </div>
                  )}

                   {step === 3 && (
                     <div className="animate-in fade-in slide-in-from-right-10 duration-500">
                        <h2 className="text-3xl font-bold mb-4">Finalizing the Experience</h2>
                        <p className="text-gray-500 mb-10">Provide your details to receive your custom quote.</p>
                        <div className="space-y-6">
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              <div className="space-y-2">
                                 ...
                                 <input type="tel" placeholder="+91 98XXX XXXXX" className="w-full bg-gray-50 border-none rounded-2xl px-5 py-4 focus:ring-2 focus:ring-[#0A1128] transition-all outline-none" />
                              </div>
                           </div>
                           <button onClick={() => alert('Architect Submission Successful')} className="w-full bg-[#0A1128] text-white py-6 rounded-2xl font-black shadow-xl shadow-blue-900/10 hover:bg-black transition-all active:scale-95">
                              Generate Proposal
                           </button>
                        </div>
                        <button onClick={() => setStep(2)} className="mt-10 text-[#0A1128] font-bold text-sm underline underline-offset-4 decoration-[#FFB800]">← Go back</button>
                     </div>
                  )}
               </div>
            </div>
