          ...
          <div className="flex items-center gap-3">
            {isDriver && !isKycFinalised && (
              <div className="flex gap-2">
                <button onClick={() => updateKycStatus('approved')} disabled={updating} className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-all disabled:opacity-50 shadow-sm">
                  <HiCheck className="w-4 h-4" /> Approve
                </button>
                <button onClick={() => { const reason = prompt('Rejection reason (optional):'); updateKycStatus('rejected', reason || undefined); }} disabled={updating} className="flex items-center gap-1.5 bg-rose-600 hover:bg-rose-700 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition-all disabled:opacity-50 shadow-sm">
