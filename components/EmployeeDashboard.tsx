            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-black uppercase tracking-widest text-slate-800">Notifications</h2>
                  {unread > 0 && (
                    <span className="bg-red-100 text-red-600 text-xs font-black px-2 py-0.5 rounded-full">{unread} new</span>
                  )}
                </div>
                <button onClick={fetchAll} className="text-slate-400 hover:text-indigo-500 transition">
                  <HiArrowPath className="w-4 h-4" />
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
                {notifications.length === 0 ? (
                  <p className="text-center text-slate-500 text-sm font-bold py-6">No notifications</p>
                ) : (
                  notifications.slice(0, 10).map(n => (
                    <div key={n.id} className={`p-3 rounded-xl border ${!n.read ? 'bg-indigo-50 dark:bg-indigo-950/30 border-indigo-200 dark:border-indigo-800' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700'}`}>
                      <div className="flex gap-2">
                        <div className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${n.read ? 'bg-gray-300' : 'bg-indigo-500'}`} />
                        <div className="flex-1">
                          <p className={`text-sm font-black ${!n.read ? 'text-gray-900 dark:text-white' : 'text-gray-600 dark:text-slate-400'}`}>{n.title}</p>
                          <p className="text-xs font-medium text-gray-500 dark:text-slate-400 mt-1 line-clamp-2">{n.message}</p>
                          <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 mt-1">
