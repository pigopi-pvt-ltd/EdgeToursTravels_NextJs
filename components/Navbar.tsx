...
import { HiXMark, HiMagnifyingGlass, HiOutlineSun, HiOutlineMoon, HiBars3, HiChevronDown } from 'react-icons/hi2';
    ...
    <header className="absolute top-0 w-full z-50">
      ...
      <nav className="w-full flex items-center justify-between px-4 md:px-10 py-4 md:py-5 text-[#0A1128] dark:text-white bg-white dark:bg-[#0A1128] shadow-sm dark:shadow-slate-900/50 transition-all duration-300">
            ...
            <div className="absolute top-full left-0 w-80 bg-transparent pt-4 opacity-0 invisible group-hover:opacity-100 group-hover:visible translate-y-2 group-hover:translate-y-0 transition-all duration-300 z-[100]">
              <div className="bg-white dark:bg-[#0F172A] rounded-lg shadow-2xl border border-gray-100 dark:border-slate-800 py-3 px-2">
                <Link href="/services/insurance" className="flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-all group/item">
                  ...
                  <span className="text-xs font-black text-[#0A1128] dark:text-white group-hover/item:text-[#EB664E] transition-colors uppercase tracking-wider">Travel Insurance</span>
                </Link>
                <Link href="/services/build-package" className="flex items-center gap-4 px-4 py-3 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-800 transition-all group/item">
                  ...
                  <span className="text-xs font-black text-[#0A1128] dark:text-white group-hover/item:text-[#EB664E] transition-colors uppercase tracking-wider">Build package</span>
                </Link>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center space-x-2 md:space-x-4">
          ...
          <button onClick={() => setIsBookingOpen(true)} className="md:hidden p-2 text-[#EB664E]">
        ...
        <div className="fixed inset-0 z-[100] md:hidden">
          ...
          <div className="absolute top-0 left-0 w-[280px] h-full bg-white dark:bg-[#0A1128] shadow-2xl flex flex-col p-6 animate-in slide-in-from-left duration-300">
            <div className="flex items-center justify-between mb-8">
              <img src="/images/logo.png" alt="Edge Tours & Travels" className="h-8 w-auto" />
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-gray-400 hover:text-black dark:hover:text-white">
            ...
            <nav className="flex-1 overflow-y-auto space-y-4">
              ...
              <div className="space-y-2">
                ...
                {expandedItems.includes('fleet') && (
                  <div className="pl-4 space-y-3 mt-2">
                    {fleetData.map(item => (
                      <div key={item.name}>
                        <Link href={item.path} className="text-xs font-bold text-[#EB664E] block mb-1">{item.name}</Link>
                        <div className="pl-3 space-y-1">
                          {item.subItems.slice(0, 3).map(sub => (
                            <Link key={sub.name} href={sub.path} className="block text-xs text-slate-500 dark:text-slate-400 py-1">{sub.name}</Link>
                          ))}
                        </div>
                      </div>
                    ))}
              ...
              <div className="space-y-2">
                <button onClick={() => toggleExpanded('services')} className="w-full flex items-center justify-between text-sm font-bold text-gray-800 dark:text-gray-200">
                  SERVICES <HiChevronDown className={`transition-transform ${expandedItems.includes('services') ? 'rotate-180' : ''}`} />
                </button>
                {expandedItems.includes('services') && (
                  <div className="pl-4 space-y-3 mt-2">
                    <Link href="/services/insurance" className="block text-xs font-bold text-gray-600 dark:text-gray-400">TRAVEL INSURANCE</Link>
                    <Link href="/services/build-package" className="block text-xs font-bold text-gray-600 dark:text-gray-400">BUILD PACKAGE</Link>
                  </div>
                )}
              </div>

              <Link href="/contact" className="block text-sm font-bold text-gray-800 dark:text-gray-200">CONTACT US</Link>
            </nav>
